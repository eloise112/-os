...
const WeiboApp: React.FC<WeiboAppProps> = ({ posts, characters, world, onAddPost, onAddComment, onToggleLike, onBack }) => {
  ...
  return (
    <div className="h-full flex flex-col bg-[#f2f2f2] animate-in slide-in-from-right duration-300 relative">
      <div className="bg-white p-4 pt-11 sticky top-0 z-20 shadow-sm border-b shrink-0">
        <div className="flex items-center justify-between mb-4">
           ...
           <div className="flex gap-6 font-bold text-[16px]">
              <button onClick={() => setActiveTab('feed')} className={activeTab === 'feed' ? 'text-black border-b-2 border-orange-500 pb-1' : 'text-gray-400 pb-1'}>推荐</button>
      ...
      <div className="flex-1 overflow-y-auto" onClick={() => setCommentingId(null)}>
        {activeTab === 'feed' ? (
           <div className="space-y-2">
             {posts.filter(p => p.platform === 'weibo').sort((a,b)=>b.timestamp-a.timestamp).map(post => {
               const author = post.authorId === 'user' ? null : characters.find(c => c.id === post.authorId);
               return (
                 <div key={post.id} className="bg-white p-4">
                    <div className="flex items-center gap-3 mb-3">
                       <img src={author?.avatar || 'https://picsum.photos/seed/user/100/100'} className="w-10 h-10 rounded-full object-cover" />
                       <div className="flex-1">
                          <div className="font-bold text-[15px]">{author?.name || '我'}</div>
                          <div className="text-[11px] text-gray-400">来自 智能终端</div>
                       </div>
                       {!author && <span className="text-[10px] text-orange-500 border border-orange-500 px-2 rounded-full font-bold">已发送</span>}
                    </div>
                      ...
                      <div className="mt-2 bg-gray-50 p-2 rounded space-y-2">
                         {post.commentsList.map(c => (
                           <div key={c.id} className="text-[13px]">
                              <span className="font-bold text-[#576b95]">{c.authorName}</span>: {c.content}
                           </div>
                         ))}
                      </div>
                    )}
           ...
           <div className="bg-white min-h-full">
              <div className="p-4 border-b font-bold text-gray-400 text-xs tracking-widest uppercase">热搜榜单</div>
              {world.hotSearches.map((item, index) => (
                 <div key={item.id} className="flex items-center gap-4 px-4 py-4 border-b active:bg-gray-50">
                    <span className={`w-6 font-bold italic text-lg ${index < 3 ? 'text-orange-500' : 'text-gray-300'}`}>{index + 1}</span>
                    <div className="flex-1">
                       <div className="font-medium text-[15px] text-gray-900">{item.title}</div>
                       <div className="text-[10px] text-gray-400">{item.hotness} 实时指数</div>
                    </div>
                    {item.tag && (
                       <span className={`text-[10px] text-white font-bold px-1.5 rounded py-0.5 ${item.tag === '爆' ? 'bg-red-500' : item.tag === '热' ? 'bg-orange-500' : item.tag === '新' ? 'bg-blue-500' : 'bg-green-500'}`}>{item.tag}</span>
                    )}
                 </div>
              ))}
           </div>
      ...
      {commentingId && (
        <div className="absolute bottom-0 w-full bg-white border-t p-2 pb-8 flex items-center gap-2 z-50">
           <input ref={commentInputRef} value={commentText} onChange={e => setCommentText(e.target.value)} onKeyPress={e => e.key === 'Enter' && handleComment()} placeholder="发布你的评论..." className="flex-1 bg-gray-100 rounded-full px-4 py-2 text-sm outline-none" />
           <button onClick={handleComment} className="text-orange-500 font-bold px-2">发送</button>
        </div>
      )}

      <div className="bg-white border-t flex justify-around p-3 pb-8 shrink-0 text-gray-300">
        <i className={`fas fa-home text-2xl ${activeTab==='feed'?'text-orange-500':''}`} onClick={()=>setActiveTab('feed')}></i>
        <i className={`fas fa-fire text-2xl ${activeTab==='hot'?'text-orange-500':''}`} onClick={()=>setActiveTab('hot')}></i>
